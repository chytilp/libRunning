...
def test_get_dates() -> None:
    _prepare_config("data/config.toml")
    trainings = get_dates(RouteModel(name="barr", description=""), version=2)
...
def test_get_date() -> None:
    _prepare_config("data/config.toml")
    training: TrainingModel = get_date(RouteModel(name="barr", description=""), "2026-01-02", version=2)
...
def test_get_sections() -> None:
    _prepare_config("data/config.toml")
    result: list[str] = get_sections(RouteModel(name="barr", description=""), version=2)
    assert result == ["1.km", "2.km", "3.km", "4.km", "5.km", "6.km", "7.km", "8.km", "9.km", "10.km"]

def test_get_aggregations() -> None:
    _prepare_config("data/config.toml")
    result: list[str] = get_aggregations(RouteModel(name="barr", description=""), version=2)
    assert result == ["1.round", "under6", "woutFilters"]

def test_get_aggregation() -> None:
    _prepare_config("data/config.toml")
    result: SectionsModel = get_aggregation(RouteModel(name="barr", description=""), "1.round",
                                            mark_date="2026-01-02", version=2)
    assert result.mark_date == "2026-01-02"
    assert sorted(list(result.date_sections.keys())) == [
        "2026-01-01", "2026-01-02", "2026-01-03"
    ]
...
def test_get_section_grades() -> None:
    _prepare_config("data/config.toml")
    result: list[GradeModel] = get_section_grades(RouteModel(name="barr", description=""), "1.km", version=2)
    assert len(result) == 5
...
def test_get_dashboard() -> None:
    ...
    result: DashboardModel = get_dashboard(RouteModel(name="barr", description=""), sections=["1.km", "2.km", "3.km", "4.km"],
                                           aggregations=["1.round"], version=2)
...
def test_get_compare() -> None:
    _prepare_config("data/config.toml")
    result: CompareModel = get_compare(RouteModel(name="barr", description=""), date_1="2026-01-01",
                                       date_2="2026-01-02", version=2)
